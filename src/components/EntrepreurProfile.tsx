...
import useFetchProfileNoPaginate from "@/requests/useFetchProfileNoPaginate";
...
const EntrepreurProfile = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedDialog, setSelectedDialog] = useState<Profile | null>(null);
  const { data: listProProfile, isLoading: loading } =
  useFetchProfileNoPaginate();    
  ...
  return (
    <div className="">
      {listProProfile?.length === 0 ? null : (
        <div className="-mt-56 xl:px-0 px-5">
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
              {Array.from({ length: skeletonCount }).map((_, index) => (
                ...
                >
                  <Skeleton className="bg-gray-200 w-full h-[350px] rounded-3xl" />
                </div>
              ))}
            </div>
          ) : (
            <div>
              <Swiper
                ...
                spaceBetween={52}
                slidesPerView={4.4}
                centeredSlides={listProProfile?.data.length <= 4}
                style={{
                  display: listProProfile?.data.length <= 5 ? "flex" : "block",
                  justifyContent:
                    listProProfile?.data.length <= 5 ? "center" : "unset",
                }}
                autoplay={listProProfile?.data.length > 5 ? { delay: 2000, disableOnInteraction: false } : false}
              ...
              >
                {listProProfile?.data?.map((profile: any) => (
              ...
              <Dialog open={isOpen} onOpenChange={closeDialog}>
                <DialogContent className="bg-custom-gradient max-w-lg max-h-[600px] rounded-xl overflow-y-auto px-0 py-0">
                  <div className="cursor-pointer absolute right-4 top-4 bg-white hover:opacity-90 flex justify-center items-center w-9 h-9 rounded-full">
                    ...
                    <div className="flex gap-6 items-start pt-6 px-6">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex gap-3">
                          <p className="text-[18px]">
                            {selectedDialog?.work_on_line == '1'?selectedDialog?.address:"Travaille en ligne"}
                          </p>
                        </div>
                    ...
                    <div className="flex flex-col gap-6 bg-background1 rounded-xl pt-6 pb-6">
                      ...
                      <div className="flex flex-col gap-3 px-6">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          {renderSocialLinks(
                            selectedDialog?.profile?.social_links
                          )}
                        </div>
                      </div>
                    </div>
...
export default EntrepreurProfile;
