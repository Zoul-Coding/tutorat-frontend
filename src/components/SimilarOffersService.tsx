            ...
            <div className="flex flex-col items-center gap-2 px-3 py-4">
              <div className="flex gap-2 justify-center items-center">
                ...
                <p className="text-white font-medium text-sm">
                  {reviews_rating??0} sur {reviews_count??0} avis
                </p>
              </div>
              <Link to={path}>
...
type OfferServiceProps = {
  dataSimilarOffers: DataOfferType | null;
};

export const SimilarOffersService: React.FC<OfferServiceProps> = ({
  dataSimilarOffers,
}) => {
  const listServiceSimilar = dataSimilarOffers?.related_offers;

  const firstRow = listServiceSimilar?.slice(
    0,
    Math.ceil(listServiceSimilar?.length / 2)
  );
  const secondRow = listServiceSimilar?.slice(
    Math.ceil(listServiceSimilar?.length / 2)
  );

  return (
    <section>
      {listServiceSimilar?.length === 0 ? (
        ""
      ) : (
        <div className="bg-roseClair mt-10 pt-10 pb-10 relative flex h-auto w-full flex-col items-center justify-center overflow-hidden rounded-lg">
          ...
          <div>
            <Marquee pauseOnHover className="[--duration:10s]">
              {firstRow?.map((review: any) => (
                <ReviewCard
                  key={review.id}
                  path={`/detail/service/${review.slug}`}
                  img={
          ...
          {/* {detailsProduct?.offer?.reviews_avg_rating}/5 sur{" "}
                                          {detailsProduct?.offer?.reviews_count} avis <div className="pointer-articles-none absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-white dark:from-background"></div>
...
export default SimilarOffersService;
