...
const AllEvenment: React.FC<AllEventProps> = ({ dataSearch }) => {
  ...
  const events =
    Array.isArray(dataSearch?.event) && dataSearch?.event.length > 0
      ? dataSearch?.event
      : dataPayload?.data;
  ...
  return (
    <section className="mb-8">
      <div className="px-5">
        {dataSearch &&
        Array.isArray(dataSearch.event) &&
        dataSearch.event.length === 0 ? (
          <p className="text-center">Aucun évenement trouvé</p>
        ) : (
          <div>
