...
type LastOffersProps = {
  dataFilter: DataSearchType | null;
  isLoading: boolean;
};

const LastOffersCategory: React.FC<LastOffersProps> = ({
  dataFilter,
  isLoading,
}) => {
  const offers = Array.isArray(dataFilter?.offers) ? dataFilter?.offers : [];
  ...
  return (
    <section className="mt-10 mb-10">
      <div className="pt-12 pb-12 px-5">
        ...
        {offers?.length === 0 ? (
          <p className="text-center pt-6">Aucun produit ou service trouvé</p>
        ) : (
          <div>
            <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 pt-6 pb-0">
              {isLoading
                ...
                : offers?.map((product: any) => (
                    <div
                      className="relative flex flex-col items-center group w-full h-[400px]"
                      key={product.id}
                    >
...
export default LastOffersCategory;
